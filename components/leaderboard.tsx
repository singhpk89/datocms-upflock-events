...
export default function LeaderBoard(){
        ...
        function getLeader() {
            ...
            .then(success => {
                ...
                success.data.forEach(element => {
                    const leader = new Leader( element.id,  element.username, element.name, element.point,element.created_at);
                    if(!data.includes(leader)){
                        data.push(leader);
                    }
                });
                setIsData(true);
        ...
        return (
            <>
                <div className={styles.grid}>
                    ...
                    <Link key={d.id} href="#">
                    <a role="button" tabIndex={0} className={cn(styles.card)}>
                        ...
                        <div className={styles.cardBody}>
                            <div>
                                ...
                                <div className="row">
                                    <h2 className="col-6 flex">{d.name}</h2>
                                    <div className="mr-1 h-24 justify-content-end col-3">{d.point}</div>
                                </div>
                                <p className="pl-2 margin-auto h-24 col-4">{d.username}</p>
                            </div>
                        </div>
                    </a>
                    </Link>
	...
	// 				<li key={i}>
	// 					<img
	// 						src={"http://www.rewards1.com/uploads/avatar/1.jpg"}
